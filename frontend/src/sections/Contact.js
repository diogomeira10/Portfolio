    ...
    return <div>
        ...
        <div className='bg-contrast rounded-lg p-4 lg:flex lg:flex-reverse lg:justify-around'>
            <form onSubmit={handleFormSubmit} className='flex flex-col gap-2 mb-8 lg:w-96'>
                ...
                <label>Email</label>
                <input placeholder='Enter your email' className={`text-background font-medium pl-1 rounded ${emptyFields.includes('email') ? 'error' : ''}`} type='text' value={email} onChange={handleEmailChange} />
                <label>Message</label>
